...
class Corrida(): 
    ...
    def buscar(self, origem, destino, paradas, servico, metodo_pagamento, solicitante):
        ...
        self.hora_inicio = datetime.now().strftime(format="%d/%m/%Y")
        self.prev_chegada_destino = self.get_prev_chegada(origem, destino)
        self.gorjeta_motorista = None
        self.status = StatusViagem.PROCURANDO_MOTORISTA.name  
        self.servico = servico
        self.motorista = None
        self.paradas = paradas
        self.solicitantes = [solicitante.id]
        self.metodo_pagamento = metodo_pagamento
        self.update_corrida()
    
    def AceitarCorrida(self, motorista):
        print(f"{motorista.nome} aceitou a corrida...")
        if self.status == StatusViagem.PROCURANDO_MOTORISTA.name:
            self.status = StatusViagem.AGUARDANDO_CONFIRMACAO_MOTORISTA.name
            self.update_corrida()
        else:
    ...
    def IniciaCorrida(self, motorista):
        ...
        self.hora_inicio = datetime.now().strftime(format="%d/%m/%Y")  
        self.motorista = motorista.id    
        self.status = StatusViagem.VIAGEM_ANDAMENTO.name
        self.update_corrida()

    def FinalizaCorrida(self, motorista):
        print(f"{motorista.nome} inicou a corrida...")
        if self.status == StatusViagem.VIAGEM_ANDAMENTO.name:
            self.status = StatusViagem.VIAGEM_CONCLUIDA.name
            self.update_corrida()
        else:
    ...
    def CancelarCorrida(self, solicitante):
        ...
        if self.status.name in (StatusViagem.PROCURANDO_LOCALIZACAO.name, StatusViagem.PROCURANDO_MOTORISTA.name, StatusViagem.AGUARDANDO_CONFIRMACAO_MOTORISTA.name):
            self.status = StatusViagem.VIAGEM_CANCELADA.name
            self.update_corrida()
        else:
